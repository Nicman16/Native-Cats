...
export default function Original() {
  ...
  // Obtener un gato aleatorio de TheCatAPI
  const getRandomCat = async () => {
    ...
    try {
      ...
      if (data[0] && data[0].breeds && data[0].breeds[0]) {
        setCatName(data[0].breeds[0].name.toUpperCase());
        setCatId(data[0].id);
      ...
      setCatId('???');
      setCatImage('');
      Alert.alert('Error', 'No se pudo obtener un gato. Intenta de nuevo.');
    }
    setLoading(false);
  };
  ...
  const renderWord = () =>
    catName.split('').map((letter, index) => (
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>Ahorcado Felino</Text>
      <Text style={styles.stats}>Ganados: {userWin} | Perdidos: {userLose}</Text>
      {loading ? (
        <ActivityIndicator size="large" />
      ) : (
        <>
          <Text style={styles.catId}>{catId}</Text>
          ...
          {gameOver && (
            <Text style={styles.lost}>💀 ¡Perdiste! Era: {catName}</Text>
          )}
...
const styles = StyleSheet.create({
  ...
  catId: { fontSize: 16, color: '#888', marginBottom: 5 },
